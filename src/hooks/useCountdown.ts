...
export default function useCountdown(onExpire?: () => void, onLast30Sec?: () => void) {
  ...
  const last30SecTriggered = useRef(false); // To prevent multiple calls
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      ...
      // Trigger last 30 seconds callback only once
  ...
  function start(seconds: number) {
    ...
    last30SecTriggered.current = false; // Reset trigger for new timer
  }
